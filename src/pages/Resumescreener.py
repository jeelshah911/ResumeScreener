...
from config import *
from utilities import save_pdfs
...
def upload_resume():
        ...
        with st.toast('Processing the uploaded data. This may take a while...'):
            for file in st.session_state.resume_uploaded:
                try:
                    ...
                    document = Document(page_content=summary_data, metadata={
                                        "Resume_Name": str(file.name)}, id=random.random())
    ...
    # TODO WHile appending to chromadb, we have to remove the file from the list of documents
                    ...
                    st.write(
                        "The uploaded resume file returns the following error message. Please check your pdf file again.")
                    st.write(error)
                    print(
                        "The uploaded resume file returns the following error message ", error)
            try:
                ...
                # with modal.container():
                st.write(
                    "The uploaded resume files couldn't save in DB. Please check the error message.")
                st.write(error)
            else:
...
def summarize_resume_openai(resume_text):
    ...
    prompts = {
        ...
        "Projects": f"Summarize the projects section of the following text:\n{resume_text}",
...
if "rag_pipeline" not in st.session_state:
    ...
    if os.path.exists(FAISS_INDEXER_DIR):
        st.session_state.vectordb = FAISS.load_local(
            FAISS_INDEXER_DIR, st.session_state.embedding_model, distance_strategy=DistanceStrategy.COSINE,  allow_dangerous_deserialization=True)
    else:
        st.session_state.vectordb = FAISS(
            st.session_state.embedding_model, distance_strategy=DistanceStrategy.COSINE)

init_vector_store()

with st.sidebar:
    st.text_input("OpenAI's API Key", type="password", key="api_key")
    st.file_uploader("Upload resumes", type=[
                     "pdf"], key="resume_uploaded", on_change=upload_resume, accept_multiple_files=True)
    st.button("Clear", on_click=clear_message)
    st.file_uploader("Upload job requirement", type=[
                     "pdf"], key="job_req_uploaded", on_change=upload_job_req)
